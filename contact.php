...
<main>
	...
	<div class="input-row">
		<label>Sujet</label> <span id="subject-info" class="info"></span><br />
	...
	<script>
		function validateContactForm() {
			...
			if (userName == "") {
				$("#userName-info").html(": Champ requis");
				$("#userName").css('border', '#e66262 1px solid');
				valid = false;
			}
			if (userEmail == "") {
				$("#userEmail-info").html(": Champ requis");
			...
			if (subject == "") {
				$("#subject-info").html(": Champ requis");
				$("#subject").css('border', '#e66262 1px solid');
				valid = false;
			}
			if (content == "") {
				$("#userMessage-info").html(": Champ requis");
